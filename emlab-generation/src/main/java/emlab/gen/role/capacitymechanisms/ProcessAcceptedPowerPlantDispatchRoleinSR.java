 ...
 */
@RoleComponent
public class ProcessAcceptedPowerPlantDispatchRoleinSR extends
	...
	public void act(StrategicReserveOperator strategicReserveOperator) {
		...
		for (Segment segment : reps.segmentRepository.findAll()) {
			...
			SegmentClearingPoint scp = reps.segmentClearingPointRepository
					.findOneSegmentClearingPointForMarketSegmentAndTime(
			...
			for (PowerPlantDispatchPlan plan : reps.powerPlantDispatchPlanRepository
					.findAllPowerPlantDispatchPlansForSegmentForTime(segment,
							getCurrentTick())) {
				// logger.warn("Entering PPDP LOOP Successfully"
				// +plan.getOldPrice());
				if (plan.getBiddingMarket().getNodeId().intValue() == market
						.getNodeId().intValue()) {
					...
					if (plan.getStatus() >= 2) {
						...
						if (plan.getSRstatus() <= -10) {
							...
							// plan.getSRstatus());
							double moneyReturned = ((plan.getAcceptedAmount()
									* scp.getPrice() * segment
									.getLengthInHours()) - ((plan
									.getAcceptedAmount() * plan.getOldPrice() * segment
									.getLengthInHours())));
							// Price mark up
