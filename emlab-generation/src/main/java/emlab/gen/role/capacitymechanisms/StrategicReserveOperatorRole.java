 ...
 */
@RoleComponent
public class StrategicReserveOperatorRole extends
		AbstractRole<StrategicReserveOperator> implements
	...
	public void act(StrategicReserveOperator strategicReserveOperator) {
		...
		// Insert a loop for zone so that strategic reserve can be turned on and
		// off for certain zones
		...
		ElectricitySpotMarket market = marketRepository
				.findElectricitySpotMarketForZone(curZone);
		// logger.warn(market.getName());
		double peakLoadforMarketNOtrend = segmentLoadRepository
				.peakLoadbyZoneMarketandTime(curZone, market);
		...
		// logger.warn(" Reserve Price " +
		// strategicReserveOperator.getReservePriceSR());
		// Contract Powers Power Plants

		// Iterable<Bid> sortedListofBidPairs =
		...
		for (Segment currentSegment : reps.segmentRepository.findAll()) {
			...
			double volumetobeContracted = strategicReserveOperator
					.getReserveVolume();
			...
			for (PowerPlantDispatchPlan currentPPDP : sortedListofPPDP) {
				...
				if (currentPPDP.getBiddingMarket().getNodeId().intValue() == market
						.getNodeId().intValue()) {
					...
					if (volumetobeContracted == 0) {
						...
						if (volumetobeContracted
								- (sumofContractedBids + currentPPDP
										.getAmount()) >= clearingEpsilon) {
							...
							double money = ((currentPPDP.getPowerPlant()
									.getTechnology().getFixedOperatingCost()))
									/ segmentCounter;
							...
							reps.nonTransactionalCreateRepository
									.createCashFlow(strategicReserveOperator,
											currentPPDP.getBidder(), money,
											CashFlow.STRRESPAYMENT,
											getCurrentTick(),
