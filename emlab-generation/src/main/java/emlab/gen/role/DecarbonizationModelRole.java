 ...
 * Main model role.
 * 
 * @author alfredas, ejlchappin, jcrichstein pcbhagwat
 * 
 */
@ScriptComponent
public class DecarbonizationModelRole extends
	...
	public void act(DecarbonizationModel model) {
		...
		for (EnergyProducer producer : reps.genericRepository
				.findAllAtRandom(EnergyProducer.class)) {
		...
		if (model.isLongTermContractsImplemented()) {
			...
			for (EnergyConsumer consumer : reps.genericRepository
					.findAllAtRandom(EnergyConsumer.class)) {
		...
		for (StrategicReserveOperator strategicReserveOperator : reps.strategicReserveOperatorRepository
				.findAll()) {
		...
		for (ElectricitySpotMarket electricitySpotMarket : reps.marketRepository
		...
		/*
		 * Process strategic reserve
		 */

		/*
		 * timerMarket.reset(); timerMarket.start();
		 * 
		 * timerMarket.stop(); logger.warn("        took: {} seconds.",
		 * timerMarket.seconds());
		 */
		...
		for (CommoditySupplier supplier : reps.genericRepository
				.findAllAtRandom(CommoditySupplier.class)) {
		...
		for (CommodityMarket market : reps.genericRepository
				.findAllAtRandom(CommodityMarket.class)) {
		...
		for (TargetInvestor targetInvestor : template
				.findAll(TargetInvestor.class)) {
		...
		if (model.isDeletionOldPPDPBidsAndCashFlowsEnabled()
				&& (getCurrentTick() - model.getDeletionAge() >= 0)) {
			timerMarket.reset();
			timerMarket.start();
			logger.warn("  9. Delete old nodes in year {}.",
					(getCurrentTick() - model.getDeletionAge()));
			reps.bidRepository.delete(reps.bidRepository
					.findAllBidsForForTime(getCurrentTick()
							- model.getDeletionAge()));
			reps.cashFlowRepository.delete(reps.cashFlowRepository
					.findAllCashFlowsForForTime(getCurrentTick()
							- model.getDeletionAge()));
			reps.powerPlantRepository.delete(reps.powerPlantRepository
					.findAllPowerPlantsDismantledBeforeTick(getCurrentTick()
							- model.getDeletionAge()));
			timerMarket.stop();
			logger.warn("        took: {} seconds.", timerMarket.seconds());
		}
		...
		timer.stop();
		logger.warn("Tick {} took {} seconds.", getCurrentTick(),
				timer.seconds());
