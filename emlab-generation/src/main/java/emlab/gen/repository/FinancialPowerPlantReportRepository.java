...
GraphRepository<FinancialPowerPlantReport> {
    ...
    @Query(value = "techName=g.v(tech).name; fr = g.v(producer).in('POWERPLANT_OWNER').as('x').out('TECHNOLOGY').propertyFilter('name',FilterPipe.Filter.EQUAL, techName).back('x').in('FINANCIALREPORT_POWERPLANT').propertyFilter('time', FilterPipe.Filter.GREATER_THAN_EQUAL, from).propertyFilter('time', FilterPipe.Filter.LESS_THAN_EQUAL, to).propertyFilter('powerPlantStatus', FilterPipe.Filter.EQUAL, 1);"
            ...
            +
            "length=fr.size(); fiveQuantile=(int)length*0.95;cvar=(long) 0;"
            + "for(int i=fiveQuantile; i<=length; i++){cvar=cvar+(fr[i].overallRevenue-fr[i].variableCosts)/fr[i].out('FINANCIALREPORT_POWERPLANT').actualNominalCapacity.next()};"
            + "cvar=cvar/((double)((length - fiveQuantile)+1));" + "return cvar;", type = QueryType.Gremlin)
    ...
    public Double calculateLowerBoundaryConventionalTech(
