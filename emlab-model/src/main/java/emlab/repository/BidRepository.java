...
public interface BidRepository extends GraphRepository<Bid> {
	...
	// supply bids in descending order
	
	@Query("START market=node({market}) MATCH (market)<-[:BIDDINGMARKET]-(bid) WHERE (bid.time = {time}) and (bid.supplyBid=true) RETURN bid ORDER BY bid.price desc")
	Iterable<Bid> findOffersDescendingForMarketForTime(@Param("market") DecarbonizationMarket market, @Param("time") long time);
