...
bigDF <- addSumOfVariablesByPrefixToDF(bigDF, "ProducerCash.Energy.Producer.")
bigDF <- addSumOfVariablesByPrefixToDF(bigDF, "ProducerCash.Renewable.Target.Investor.")
originalDF<-bigDF

runIds=character()
for(i in seq(151,300)){
  runIds=c(runIds,paste("bigStochasticTest-",i,"-1", sep=""))
}

bigDF<-originalDF[originalDF$runId%in%runIds,]

co2PricePlot<-plotTimeSeriesWithConfidenceIntervalByFacettedGroup(bigDF, "CO2Auction", "CO2 Price")
co2PricePlot

co2SpaghettiPrice<-plotSpaghettiTimeSeries(df=bigDF[bigDF,],variable="CO2Auction",ylabel="CO2 Price [EUR/ton]")+
...
co2EmissionsSpaghetti<-co2EmissionsSpaghetti+geom_line(aes(y=CO2CapinTonpA_CO2.cap),linetype = 2, colour="red", size=1)
...
co2EmissionPlot<-co2EmissionPlot+geom_line(aes(y=CO2CapinTonpA_CO2.cap),linetype = 2, colour="red", size=1)
co2EmissionPlot
...
spaghettiPricePlotinA<-plotTimeSeriesWithConfidenceIntervalByFacettedGroup(df=bigDF,variable="Avg_El_PricesinEURpMWh_Country.A","Avg. Electricity Price in CWE [EUR/MW]")
spaghettiPricePlotinA
...
spaghettiPricePlotinB<-plotSpaghettiTimeSeries(df=bigDF,variable="Avg_El_PricesinEURpMWh_Country.B","Avg. Electricity Price in Country GB [EUR/MW]")+
  ylim(0,100)
spaghettiPricePlotinB

aggregateCashEnergyProducers<-plotTimeSeriesWithConfidenceIntervalByFacettedGroup(bigDF, "ProducerCash.Energy.Producer.Sum", "Aggregated Energy Producer Cash")
aggregateCashEnergyProducers

aggregateCashTargetInvestors<-plotTimeSeriesWithConfidenceIntervalByFacettedGroup(bigDF, "ProducerCash.Renewable.Target.Investor.Sum", "Aggregated Target Investor Cash")
aggregateCashTargetInvestors
...
#--------------Volatility------------------------
bigDF$co2PriceA<-pmax(bigDF$NationalMinCO2PriceinEURpTon_Country.A, bigDF$CO2Auction,bigDF$EUCO2PriceFloor_EU.CO2.price.floor)
bigDF$co2PriceB<-pmax(bigDF$NationalMinCO2PriceinEURpTon_Country.B, bigDF$CO2Auction,bigDF$EUCO2PriceFloor_EU.CO2.price.floor)
...
#moltenCO2Prices$modelRun<-factor(moltenCO2Prices$modelRun, levels=c("PureETS","BothMin", "MinCO2", "MinCWE","MinGB", "PriceCeiling"))
moltenCO2Prices$variable<-renamer(moltenCO2Prices$variable, "co2PriceA", "CWE")
moltenCO2Prices$variable<-renamer(moltenCO2Prices$variable, "co2PriceB", "GB")
moltenCO2Prices$variable<-factor(moltenCO2Prices$variable, levels=c("CWE","GB"))
...
ddply(moltenCO2Prices, .variables=c("modelRun", "variable"), .fun=function(df, columnname) median(df[,columnname]), "value")
