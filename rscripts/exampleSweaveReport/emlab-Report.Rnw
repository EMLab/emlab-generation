...
headlessOutputFolder<-"~/Desktop/emlabGen/output/"
headlessModelRunName<-"rScriptTest"
noOfTicks<-40
...
if(exists("scenario") & !dryRun){if(!is.null(scenario)) message(paste("Load Scenario",scenario,loadScenario(scenario)))}
setwd(outputFolder)
if(!dryRun){
  runResult<-runSimulation(createReportPdf,noOfTicks, runName)
} else{
  if(headlessRun){
    getResultListFromHeadlessModelRun(headlessOutputFolder,headlessModelRunName,runName)
  }
